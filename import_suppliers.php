    ...
    if (!is_uploaded_file($_FILES['file']['tmp_name'])) {
        ...
        if (!$handle) { $error = 'Não foi possível ler o arquivo.'; }
            ...
            while (($row = fgetcsv($handle, 0, $delimiter)) !== false) {
                ...
                $ok = Supplier::create($name, $contact ?: null, $city ?: null, $phone ?: null);
...
<div class="flex flex-col gap-6">
  <div class="flex items-center justify-between">
    <h1 class="text-2xl font-semibold text-brand-900">Importar fornecedores (CSV)</h1>
    <a href="suppliers.php" class="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-brand-700 text-white hover:bg-brand-800">
      <i data-lucide="truck" class="w-5 h-5"></i> Voltar a Fornecedores
    </a>
  </div>
...
render_layout('Importar fornecedores', $content);
?>